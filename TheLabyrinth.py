...
FONT4 = pg.font.SysFont('Comic Sans MS', 20, bold=True)
...
def crea_labirinto(rows, cols):
    ...
    SCREEN.blit(loading, (WIDTH/2-100, HEIGHT/2-50))
    aggiorna()
...
def menu_iniziale():
    ...
    SCREEN.blit(titolo, (WIDTH/8, 20))
    modalita = FONT3.render("Press the difficulty:", True, WHITE)
    SCREEN.blit(modalita, (WIDTH/4, 200))
    facile = FONT3.render("1) Easy", True, WHITE)
    SCREEN.blit(facile, (WIDTH/4, 250))
    medio = FONT3.render("2) Medium", True, WHITE)
    SCREEN.blit(medio, (WIDTH/4, 300))
    difficile = FONT3.render("3) Hard", True, WHITE)
    SCREEN.blit(difficile, (WIDTH/4, 350))
    Molto_difficile = FONT3.render("4) Very hard", True, WHITE)
    SCREEN.blit(Molto_difficile, (WIDTH/4, 400))
...
while True:
    ...
    while flag!=0:
        ...
        titolo = FONT1.render("THE LABYRINTH", True, WHITE)
        SCREEN.blit(titolo, (WIDTH/8, 20))
        esci = FONT4.render("0) Back to menu", True, WHITE)
        SCREEN.blit(esci, (DISTANZA_DA_LATO+(COLS*DIM_MURI-170), DISTANZA_DA_TITOLO+(ROWS*DIM_MURI)-10))
        instructions = FONT4.render("To move use ARROWS or WASD", True, WHITE)
        SCREEN.blit(instructions, (DISTANZA_DA_LATO, DISTANZA_DA_TITOLO+(ROWS*DIM_MURI)-10))
        ...
        if x==arrivo and y==COLS-1:
            vittoria = FONT2.render("Congratulations, YOU ESCAPED!!!", True, AZZURRO)
            SCREEN.blit(vittoria, (60, 160+(ROWS*DIM_MURI)+10))
            aggiorna()
            ricominciamo = False
